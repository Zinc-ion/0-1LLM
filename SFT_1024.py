...
def train_epoch(epoch, wandb):
     ...
     for step, (X, Y, loss_mask) in enumerate(train_loader):
          ...
          if (step + 1) % args.save_step == 0:
               model.eval()
               ckp = f'{args.save_dir}/SFT_1024_train.pth'
...
def init_model(lm_config):
     ...
     # 加载预训练权重
     ckp = f'./results/SFT_train_mini512.pth'  # 预训练模型路径
...
if __name__ == "__main__":
     ...
     # ------------------ 训练准备 ------------------
     # 初始化WandB运行（如果启用）
     args.wandb_run_name = f"SpongeBob-SFT1024-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
     # 混合精度训练上下文
     ...
     model.eval()
     # 训练结束后保存最终模型
     final_ckp = os.path.join(args.save_dir, "SFT_final_1024_eval.pth")

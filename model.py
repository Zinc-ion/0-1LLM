...
class SpongeBob(PreTrainedModel):
     ...
     def _stream(self, input_ids, eos_token_id, max_new_tokens, temperature, top_p, rp, use_cache, **args):
            ...
            # while循环条件中的张量维度检查：input_ids是二维张量 [batch_size, seq_len]
            while input_ids.shape[1] < max_new_tokens - 1:
                ...
                if first_seq or not use_cache:
                    ...
                    # 张量切片语法：input_ids[:, -1:] 表示取所有batch的最后1个token（保持二维）
                    out = self(input_ids[:, -1:], past_key_values=past_kvs, use_cache=use_cache,
                            start_pos=input_ids.shape[1] - 1, **args)
                    # start_pos参数动态计算：input_ids.shape[1] - 1 获取当前序列长度
                # 取出最后一步的logits及更新后的KV缓存
                logits, past_kvs = out.logits[:, -1, :], out.past_key_values
                # 对已经生成的token进行惩罚，防止重复生成
                # 列表推导式 + 集合去重：list(set(input_ids.tolist()[0])) 获取已生成的不重复token
